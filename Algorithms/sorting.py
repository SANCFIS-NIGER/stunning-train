...
def check_execution_time(list_to_process, dict_to_process):
    ...
    dict_to_process : The dictionary containing the function to be processed, the key is the function name
    ...
    bubble_sort(list_to_process)
    end_time = time.time()
    function_speed = end_time - start_time
    dict_to_process["Bubble "] = function_speed

    start_time = time.time()
    merge_sort(list_to_process)
    end_time = time.time()
    function_speed = end_time - start_time
    dict_to_process["Merge "] = function_speed

    start_time = time.time()
    insertion_sort(list_to_process)
    end_time = time.time()
    function_speed = end_time - start_time
    dict_to_process["Insertion "] = function_speed

    start_time = time.time()
    shell_sort(list_to_process)
    end_time = time.time()
    function_speed = end_time - start_time
    dict_to_process["Shell "] = function_speed

    start_time = time.time()
    selection_sort(list_to_process)
    end_time = time.time()
    function_speed = end_time - start_time
    dict_to_process["Selection "] = function_speed

    start_time = time.time()
    heap_sort(list_to_process)
    end_time = time.time()
    function_speed = end_time - start_time
    dict_to_process["Heap "] = function_speed

    return dict_to_process


def display_speed():
    dict_of_speed_results = {}  # Initializing the dictionary to store the data
    check_execution_time(random_list(0, 10000, 3, 10), dict_of_speed_results)
    ...
    for key, value in sorted_dict_of_speed_results.items():  # Going throughout the sorted dictionary
        v_function = key
        v_speed = value
        print("{:<25} {:<25}".format(v_function, v_speed))
